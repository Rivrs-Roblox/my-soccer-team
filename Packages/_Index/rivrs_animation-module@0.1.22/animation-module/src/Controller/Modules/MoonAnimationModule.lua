...
function MoonAnimationModule:_setupParticleEmitters(_, item)
	...
	if part:IsA("Bone") then
		...
		-- For regular parts, create or use an existing Attachment
...
function MoonAnimationModule:_handleParticleEmitterBehavior(index, behaviorType, behavior)
	local emitter
	if self.particleEmitters[self.animationData.Items[index].Name] then
		emitter = self.particleEmitters[self.animationData.Items[index].Name]
	else
		-- Search for the emitter in BaseParts
		for _, part in pairs(self.baseParts) do
			emitter = part:FindFirstChild(self.animationData.Items[index].Name, true)
	...
	if emitter then
		...
		-- else
		--warn("ParticleEmitter not found for index:", index)
	end
end
...
function MoonAnimationModule:destroy()
	...
	for _, emitter in pairs(self.particleEmitters) do
		...
		emitter:Destroy()
		if parent:IsA("Attachment") and #parent:GetChildren() == 0 then
			parent:Destroy()
		end
	end
	for _, part in pairs(self.baseParts) do

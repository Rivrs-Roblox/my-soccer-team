...
function SoundModule:preload()
	...
	if self.soundData.RollOffMaxDistance then
		self.sound.RollOffMaxDistance = self.soundData.RollOffMaxDistance
	end
	if self.soundData.RollOffMinDistance then
		self.sound.RollOffMinDistance = self.soundData.RollOffMinDistance
	end
	if self.soundData.EmitterSize then
		self.sound.EmitterSize = self.soundData.EmitterSize
	end
...
function SoundModule:createChildSound()
	...
	if self.soundData.RollOffMaxDistance then
		childSound.RollOffMaxDistance = self.soundData.RollOffMaxDistance
	end
	if self.soundData.RollOffMinDistance then
		childSound.RollOffMinDistance = self.soundData.RollOffMinDistance
	end
	if self.soundData.EmitterSize then
		childSound.EmitterSize = self.soundData.EmitterSize
	end
...
function SoundModule:play()
	...
	if self.isSpammable then
		...
		if self.fadeTime > 0 then
			local originalVolume = self.volume
			self.sound.Volume = 0
			self.sound:Play()
...
function SoundModule:setParent(parent)
	self.parent = parent
	if self.sound then
		self.sound.Parent = parent
	end
	for _, childSound in ipairs(self.childSounds) do
		childSound.Parent = parent
	end
end

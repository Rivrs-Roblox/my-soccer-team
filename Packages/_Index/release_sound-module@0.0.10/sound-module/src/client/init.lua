...
function SoundCtrl:EnhanceSoundData(soundData, soundName)
	...
	if enhanced.Category == self.Categories.UI then
		...
		enhanced.Spammable = enhanced.Spammable ~= nil and enhanced.Spammable or true
		enhanced.StopPreviousSounds = enhanced.StopPreviousSounds ~= nil and enhanced.StopPreviousSounds or true
		enhanced.FadeTime = enhanced.FadeTime or 0
...
function SoundCtrl:GetOrCreateSound(soundName, parent)
	...
	if preloadedSound then
		if preloadedSound.sound and not preloadedSound.sound.Parent then
			-- It was destroyed externally, remove from cache
			for k, v in pairs(self.Sounds) do
				if v == preloadedSound then
					self.Sounds[k] = nil
				end
			end
		else
	...
	if self.Sounds[key] then
		...
		if sound.sound and not sound.sound.Parent then
			-- It was destroyed externally, clear cache to recreate
			self.Sounds[key] = nil
		else
			if parent then

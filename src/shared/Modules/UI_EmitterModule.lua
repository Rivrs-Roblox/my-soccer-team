...
game:GetService("RunService")
	:BindToRenderStep("UI_ParticleModule", Enum.RenderPriority.First.Value + 1, function(deltaTime)
		RunTime = tick() - t0
		...
		for _, Data in pairs(Emitters) do
			if not Data.Emitter:IsDescendantOf(game) then
		...
		for _, Data in pairs(toRemove) do
			table.remove(Emitters, table.find(Emitters, Data))
		end
		...
		for _, Particle in pairs(ParticleQueue) do
			...
			if ParticleRunTime >= Particle.Lifetime then
				...
				local SpeedUDim = UDim2.new(
					0,
					math.sin(math.rad(Angle)) * Particle.SpeedDisplacement,
					0,
					-math.cos(math.rad(Angle)) * Particle.SpeedDisplacement
				)

				Particle.Accel = Particle.Accel
					+ Particle.Emitter.Acceleration
						* 100
						* Particle.Scale
						* (ParticleRunTime ^ 2)
						* deltaTime
				...
				Particle.UI.Position = Particle.StartPos
					+ SpeedUDim
					+ UDim2.new(0, Particle.Accel.X, 0, -Particle.Accel.Y)
			end
		end
...
function addParticle(ParticleEmitter, UI_Parent, Scale)
	if not UI_Parent or not UI_Parent:IsDescendantOf(game) then
		return
	end
	...
	for _, oldUI in pairs(Garbage) do
		local index = table.find(Garbage, oldUI)
		local reusableUI = oldUI.UI

		if reusableUI then
			local canReuse = pcall(function()
				reusableUI.Parent = UI_Parent
			end)

			if index then
				table.remove(Garbage, index)
			end
	...
	table.insert(
		ParticleQueue,
		{
			...
			Transparency = RandomizedTransparency,
			Size = RandomizedSize,
			Speed = ParticleSpeed,
			RotSpeed = ParticleRotSpeed,
...
function getRandomNumberSequence(NumberSeq)
	...
	for _, kp in pairs(NumberSeq.Keypoints) do
		table.insert(
			newSeq.Keypoints,
			{ Time = kp.Time, Value = r_num(kp.Value - kp.Envelope, kp.Value + kp.Envelope) }
		)
	end

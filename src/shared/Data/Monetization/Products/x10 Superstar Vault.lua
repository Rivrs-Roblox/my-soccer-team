...
return table.freeze({
	[3579687590] = { -- REPLACE WITH ACTUAL PRODUCT ID
		["Name"] = "x10 Superstar Vault",
		["BeforeCheck"] = function(self, userId)
			...
			if GetTableLength(data.Inventory.SoccerCharacters) + 10 >= data.Inventory.Storage.Stored then
		...
		["Purchased"] = function(self, userId)
			...
			GachaService:OpenGacha(player, "SoccerCharacters", "11", 10)
		end,
